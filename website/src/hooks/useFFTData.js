...
function useFFTData(_a) {
    ...
    (0, react_1.useEffect)(function () {
        ...
        var updateFrequencyData = function () {
            ...
            var usableRange = Math.floor(bufferLength * 0.3); // Use only 30% of spectrum
            var binsPerBar = Math.max(1, Math.floor(usableRange / binCount));
            for (var i = 0; i < binCount; i++) {
                var start = i * binsPerBar;
                var end = Math.min(start + binsPerBar, usableRange);
                var sum = 0;
                var count = 0;
                ...
                var boosted = Math.min(255, average * 2.0);
